...
class ParagraphBlocktype(build.NotionObject, BlockTypeObjects):
    ...
    """
    __slots__: typing.Sequence[str] = ('_rich_text')
    
    def __init__(self, rich_text_object: RichTextPropertyValue, /, *, children: list[BlockTypeObjects] | None = None, 
                 block_color: ColorEnum | str | None = None):# ColorEnum.default.value) -> None:
        super().__init__()

        self._rich_text = build.NotionObject()
        # need to try 'type':'rich_text' key in paragraph
        self._rich_text.set('rich_text', rich_text_object['rich_text'])

        self.set('type', 'paragraph')
        self.set('paragraph', self._rich_text) 
        self.set('color', block_color) if block_color else None
...
class CodeBlocktype(build.NotionObject, BlockTypeObjects): 
    ...
    __slots__: typing.Sequence[str] = ('_code')
    
    def __init__(self, 
                 rich_text_object: RichTextPropertyValue, 
                 language: CodeEnum | str,
                 /, *, 
                 caption: Caption | None = None) -> None:
        super().__init__()
        self._code = build.NotionObject()
        self._code.set('rich_text', rich_text_object['rich_text'])
        self._code.set('language', language)
        self._code.set('caption', caption) if caption else None
